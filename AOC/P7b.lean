...
def Hand.getType (h : Hand) : HandTypes :=
  ...
  let numJokers := m.findD Card.joker 0
  let m := m.erase Card.joker
  if m.size == 1 || m.size == 0 then HandTypes.fiveOfAKind
  else if m.size == 2 then
    let v := m.toList |>.map (λ (_, v) => v) |>.maximum? |>.get!
    if (v+numJokers) == 4 then HandTypes.fourOfAKind
    else HandTypes.fullHouse
  else if m.size == 3 then
    let v := m.toList |>.map (λ (_, v) => v) |>.maximum? |>.get!
    if (v+numJokers) == 3 then HandTypes.threeOfAKind
    else HandTypes.twoPairs
  else if m.size == 4 || numJokers == 1 then HandTypes.onePair
  else
  HandTypes.highCard
